...
service (``sentinel/ledger/shim/``) built on the official ``@hyperledger/fabric-gateway``
package. It exposes the four chaincode transactions over local HTTP/JSON; this client
is a thin, dependency-free wrapper that implements :class:`~sentinel.pdp.queue.LedgerSink`.

    node sentinel/ledger/shim/server.js          # after ./network.sh up + deployCC
    SENTINEL_LEDGER=fabric uvicorn sentinel.pdp.app:app

Commit failures surface as :class:`~sentinel.ledger.sim.LedgerRejected` with the
...
from sentinel.ledger.sim import LedgerRejected

DEFAULT_URL = os.environ.get("SENTINEL_FABRIC_SHIM", "http://127.0.0.1:7071")
