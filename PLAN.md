...
**Deliverables:**
- [ ] `ztb/features/cert_mapper.py` — maps CERT rows to CloudTrail-style JSON events per the mapping in CLAUDE.md. Preserves timestamps, user IDs, OU → bucket assignment.
- [ ] `ztb/features/labels.py` — attaches malicious labels from `answers/` (scenario files).
- [ ] `ztb/features/baseline.py` — per-principal profile store (dict/SQLite locally, DynamoDB in cloud mode), EWMA update with 30-day half-life.
- [ ] `ztb/features/builder.py` — builds the 34-dim vector for one event given the principal's baseline and the sliding-window state. Standardisation stats fitted on train window only.
...
- [ ] `ztb/risk/autoencoder.py` — PyTorch model 34-24-16-8 mirrored; train benign only; save `models/ae.pt`.
- [ ] `ztb/risk/iforest.py` — sklearn IsolationForest(200, 256); save pickle.
- [ ] `ztb/risk/fusion.py` — empirical CDF calibration on held-out benign set; `score(x) -> r`.
- [ ] `ztb/risk/trust.py` — eq. (4) + Table III `band_lookup(R) -> (verdict, ttl, scope)`.
...
- [ ] `ztb/pdp/app.py` — FastAPI. `POST /authorize` implements Algorithm 1 exactly (static check → features → score → trust → verdict → credential → record → async enqueue → baseline update).
- [ ] `ztb/pdp/static_policy.py` — simple RBAC/ABAC JSON policy evaluator (stand-in for IAM in local mode).
- [ ] `ztb/pdp/credentials.py` — local mode: signed mock token with TTL/scope; cloud mode: `sts.assume_role` with session policy.
- [ ] `ztb/pdp/signer.py` — ECDSA P-256 key gen, sign/verify over canonical JSON of the record tuple.
- [ ] `ztb/pdp/chain.py` — per-principal seq + prevHash; `h_feat = sha256(x || salt)`; off-chain evidence store (encrypted file).
- [ ] `ztb/pdp/queue.py` — in-memory/asyncio queue + committer worker (ledger client plugged in Module 4; until then, append-only JSONL).
...
- [ ] `ztb/ledger/client.py` — Fabric Gateway SDK (Python via gRPC or thin Go/Node shim); committer drains queue → `LogAccess`.
