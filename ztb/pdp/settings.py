...
from ztb.config import FUSION_ALPHA, MODELS, ROOT
...
@dataclass
class Settings:
    mode: str = field(default_factory=lambda: os.environ.get("ZTB_MODE", "local"))
    models_dir: Path = field(default_factory=lambda: Path(os.environ.get("ZTB_MODELS", MODELS)))
    seed: int = field(default_factory=lambda: int(os.environ.get("ZTB_SEED", "0")))
    alpha: float = FUSION_ALPHA
    policy_path: Path = field(
        default_factory=lambda: Path(os.environ.get("ZTB_POLICY", DEFAULT_POLICY)))
    ...
    sts_role_arn: str = field(default_factory=lambda: os.environ.get("ZTB_STS_ROLE_ARN", ""))
